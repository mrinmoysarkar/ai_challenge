...
class SampleHazardDetector(IDataReceived):
    ...
    def dataReceived(self, lmcpObject):
        if isinstance(lmcpObject,KeepInZone):
            ...
            if veicleid in self.__uavsisinfirezone and (self.__currentHeadingAngleUAV[veicleid] == 0 or self.__currentHeadingAngleUAV[veicleid] == 2):
                if (self.__simulationTimeSeconds-self.__insideFireZoneLastTime[veicleid])>240:
                    ...
                    self.__insideFireZoneLastTime[veicleid] = self.__simulationTimeSeconds
                    self.turn(veicleid)
                    self.__turnlocation[veicleid] = veicleLocation
                    self.__turntime[veicleid] = self.__simulationTime
                    self.__option[veicleid-1] = 1
                elif self.isLeavingFireZone(veicleid,veicleLocation):
                    if self.__option[veicleid-1] == 0:
                        ...
                        if (self.__simulationTime - self.__turntime[veicleid]) > 3.3:
                            self.turn(veicleid)
                            self.__turnlocation[veicleid] = veicleLocation
                            self.__turntime[veicleid] = self.__simulationTime
                            self.__option[veicleid-1] = 1
            ...
            if not veicleid in self.__uavsLoiter:
                ...
                # self.sendLoiterCommand(veicleid,veicleLocation,(self.__searchAreaWidth/self.__noOfUAVs))
                self.sendHeadingAngleCommand(veicleid,315)
            if not veicleid in self.__uavsgothint and self.__gotHint and not veicleid in self.__uavsisinfirezone:
                ...
                self.sendLoiterCommand(veicleid, self.__firezoneHintLocation,500)
    ...
    def sendLoiterCommand(self, vehicleId, location, radius):
        ...
        loiterAction.set_Direction(LoiterDirection.CounterClockwise)
