...
class SampleHazardDetector(IDataReceived):
    ...
    def dataReceived(self, lmcpObject):
        ...
        if isinstance(lmcpObject, KeepInZone):
            ...
            if fireZoneType == HazardType.Fire:
                self.__maxSpeedofUAVduringSurvey[detectingEntity] = self.__maxSpeedofUAV[detectingEntity] if self.__maxSpeedofUAV[detectingEntity] != self.__maxSpeedGlobal else self.__maxSpeedForsurvey ## play here

                self.__uavsInSarvey[detectingEntity] = True
                # if self.__uavWasSurveying and (detectingEntity in self.__uavWasSurveying):
                #     pass
                # else:
                self.__uavWasSurveying[detectingEntity] = detectedLocation
                    # print(self.__uavWasSurveying)
                self.__gotHint = True
            
                [x,y] = self.convertLatLonToxy(detectedLocation.get_Latitude(),detectedLocation.get_Longitude())
                # self.__uavWasSurveying[detectingEntity] = [x,y]
    ...
    def calculateGridCoordinate(self):
        ...
        for ws in range(wSeg):
            for hs in range(hSeg):
                ...
                n = 10
                delsegy = dh/n
                curx,cury = currCenterx,currCentery
                
                wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx+dw,cury,2,0)
                waypoints = waypoints + wpoints
                curx,cury=curx+dw,cury
                wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx,cury+delsegy,waypointNumber,0)
                waypoints = waypoints + wpoints
                curx,cury=curx,cury+delsegy
                for cli in range(n-1):
                    if cli%2==0:
                        wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx-dw,cury,waypointNumber,0)
                        waypoints = waypoints + wpoints
                        curx,cury=curx-dw,cury
                        wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx,cury+delsegy,waypointNumber,0)
                        waypoints = waypoints + wpoints
                        ...
                        waypoints = waypoints + wpoints
                        curx,cury=curx,cury+delsegy
                wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,currCenterx,currCentery,waypointNumber,1)
                waypoints = waypoints + wpoints
                ...
                wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx+ddw,cury,waypointNumber,0)
                waypoints = waypoints + wpoints
                curx,cury=curx+ddw,cury

                wpoints,waypointNumber = self.getBetweenLatLonwithoutVIDAlt(curx,cury,curx,cury+ddh,waypointNumber,0)
                waypoints = waypoints + wpoints
                curx,cury=curx,cury+ddh
    ...
    def polar_angle(self,p0,p1=None):
        if self.__debug:
            print("polar_angle enter")
        if p1==None: 
            p1=self.anchor
        y_span=p0[1]-p1[1]
        x_span=p0[0]-p1[0]
        if self.__debug:
            print("polar_angle exit")
    ...
    def mergeFireZones(self, allxypoints):
        ...
        clustering = MeanShift(bandwidth=1).fit(allxypoints)
        ...
        # print(pointsInZones)
        if self.__debug:
            print("mergeFireZones exit")
        return dict(pointsInZones)

    def updateSamples(self,sample,r,Np):
        if self.__debug:
            print("updateSamples enter")
    ...
    def assignNewMission(self,vstate):
        ...
        if vid in self.__uavsInSarvey and self.__uavsInSarvey[vid]:
            ...
            self.sendServeyCommand(vstate,0,r=(self.__searchCircleRadius+1000))
        # elif self.__uavWasSurveying and vid in self.__uavWasSurveying and vid in self.__uavisHeadingtoSurveylocation and not self.__uavisHeadingtoSurveylocation[vid]:
        #     surveyPos = self.__uavWasSurveying[vid]
        #     self.__uavisHeadingtoSurveylocation[vid] = True
        #     self.sendWaypoint(vid, vstate.Location, surveyPos , radius=1000,torecharge=False)
    ...
    def checkpowerStatus(self,AirVehicleState):
        ...
        if EnergyRate > 0.0:
            ...
            MaximumDistLeft = Speed * RemainTime - 3000
            # if veicleid == 4:
            ...
            if Dist[MinIndice] >= MaximumDistLeft:
                ...
                radius = radius*0.95
                # print(self.__uavWasSurveying)
                if self.__uavWasSurveying and (veicleid in self.__uavWasSurveying):
                    # print("i was in survey")
                    self.__uavWasSurveying[veicleid] = Location#[x,y]
    ...
    def getNewAreaforSearch(self,vstate): # needs work 
        ...
        if flag:
            ...
            waypoints = waypoints1 + waypoints
            vid = vstate.ID
            if self.__uavWasSurveying and vid in self.__uavWasSurveying:
                surveyPos = self.__uavWasSurveying[vid]
                self.sendWaypoint(vid, vstate.Location, surveyPos , radius=1500,torecharge=False)
            else:
                self.sendOnlywaypoints(vstate,waypoints,firstWaypoint=minLocid)
            if self.__debug:
                print("getNewAreaforSearch exit")
