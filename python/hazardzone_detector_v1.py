    ...
    def dataReceived(self, lmcpObject):
        if isinstance(lmcpObject,KeepInZone):
            ...
            if veicleid in self.__uavsisinfirezone and (self.__currentHeadingAngleUAV[veicleid] == 0 or self.__currentHeadingAngleUAV[veicleid] == 2):
                if not self.isinKeepInZone(veicleLocation) and (self.__simulationTime-self.__keepoutoptionChangetime[veicleid-1])>7:
                ...
                if self.isLeavingFireZone(veicleid,veicleLocation):
                    if self.__option[veicleid-1] == 0:
                        ...
                        if (self.__simulationTime - self.__turntime[veicleid]) > 3.3:#self.getdistance(veicleLocation,self.__turnlocation[veicleid])>50000000:
                            self.turn(veicleid)
                            self.__turnlocation[veicleid] = veicleLocation
                            self.__turntime[veicleid] = self.__simulationTime
                            self.__option[veicleid-1] = 1
            if veicleid in self.__uavsisinfirezone:
                if (self.__currentHeadingAngleUAV[veicleid] == 1 or self.__currentHeadingAngleUAV[veicleid] == 3) and (self.__simulationTime - self.__turntime[veicleid]) > 1.3:#self.getdistance(veicleLocation,self.__turnlocation[veicleid])>27000:
                    self.turn(veicleid)
                    self.__turnlocation[veicleid] = veicleLocation
                    self.__turntime[veicleid] = self.__simulationTime
            ...
            if not veicleid in self.__uavsLoiter:
                ...
                self.sendLoiterCommand(veicleid,self.__centerLocation,(self.__searchAreaWidth/self.__noOfUAVs)*veicleid)
